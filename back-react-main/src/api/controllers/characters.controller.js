...
const getOneCharacter = async (req, res) => {
    try {
        const {id} =req.params;
        const oneCharacter = await Character.findById(id).populate('planet').populate('country');
...
const putCharacter = async (req, res) => {
    try {
        ...
        const updateCharacters = await Character.findByIdAndUpdate(id, putCharacters, {new: true});
        if(!updateCharacters){
...
module.exports = {getCharacters, getOneCharacter, postCharacter, putCharacter,deleteCharacter};